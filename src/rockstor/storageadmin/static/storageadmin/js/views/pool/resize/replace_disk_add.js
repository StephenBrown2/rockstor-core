...
PoolReplaceDiskAdd = RockstorWizardPage.extend({
    ...
    renderDisks: function () {
        ...
        this.$('#checkAll').closest('th').replaceWith('Select One');
    ...
    clickCheckbox: function (event) {
        $(".diskid").click(function(){
            $(this).closest('tr').toggleClass('row-highlight', this.checked);
            $(".diskid").not(this).prop("checked", false);
        });
    },
     ...
     */
    save: function () {
        var valid = this.$('#force-replace-form').valid();
        if(!valid){
        ...
        this.model.set('addDisk', diskIds[0]);
